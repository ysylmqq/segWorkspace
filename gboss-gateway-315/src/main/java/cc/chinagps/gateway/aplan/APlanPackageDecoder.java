...
public class APlanPackageDecoder implements InputStreamDecoder{
	private InputPackageHandler handler;
	
	public APlanPackageDecoder(InputPackageHandler handler){
	...
	private int calTotalSize(){
		...
		int totalSize = Util.getInt(buff.array());
		...
		while((leftBytes = (buff.limit() - buff.position())) > 0){
			...
			if(headNeed > 0){
				if(leftBytes < headNeed){					
					...
					if(totalSize < HEAD_SIZE || totalSize > 8192){
						reset();
	...
	public static void main(String[] args) {
		APlanPackageDecoder d = new APlanPackageDecoder(null);
		byte[] h1 = {1};
		byte[] h2 = {2, 3};
		byte[] h3 = {4, 5, 6};
