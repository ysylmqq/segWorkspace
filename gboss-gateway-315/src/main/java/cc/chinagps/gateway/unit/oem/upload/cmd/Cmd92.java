 ...
 * 非法开门报警
 */
public class Cmd92 implements UploadHandler {
	private Logger logger_debug = Logger.getLogger(LogManager.LOGGER_NAME_DEBUG);

	@Override
	public boolean handlerPackage(OEMPackage pkg, UdpServer server, UnitSocketSession session) throws Exception {
		// TODO Auto-generated method stub
		byte[] body = pkg.getData();
		OEMGPSInfo gps = OEMGPSInfo.parse(body, 0, true);
		logger_debug.debug("OEM Cmd92 " + gps);

		OEMUploadUtil.handleGPS(server, session, pkg, gps);

		OEMUploadUtil.handlerAlarm(server, session, pkg, gps);

		// ack 8801D
		byte[] data = OEMPkgUtil.getCommonResponseToUnit(session, (byte) 0x20, (byte) 0x92, pkg);
