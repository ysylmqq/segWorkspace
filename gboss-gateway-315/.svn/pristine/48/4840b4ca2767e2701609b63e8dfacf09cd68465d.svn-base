...
public class Cmd93 extends CheckLoginHandler{
	...
	public boolean handlerPackageSessionChecked(SegPackage pkg, UnitServer server,
		...
		if(strBody.startsWith("(ITV")){
			...
			return true;
		}else if(strBody.startsWith("(FNS,ITV,22")){
			//开启/关闭熄火定时报告应答
			handlerUploadGPSByAccOffDeliver(pkg, server, session, strBody);
			return true;
	 ...
	 * 开启/关闭熄火定时报告应答
	 */
	public static void handlerUploadGPSByAccOffDeliver(SegPackage pkg, UnitServer server,
			UnitSocketSession session, String strBody){
		if(logger_debug.isDebugEnabled()){
			logger_debug.debug("开启/关闭熄火定时报告应答...:" + pkg.getSerialNumber());
		}
		
		//String sn = pkg.getSerialNumber();
		String callLetter = session.getUnitInfo().getCallLetter();
		int flag = Integer.valueOf(strBody.substring(12, 13));
		int cmdId = flag == 0? CmdUtil.CMD_ID_STOP_ACC_OFF_DELIVERY: CmdUtil.CMD_ID_START_ACC_OFF_DELIVERY;
		String sn = CmdUtil.getCmdCacheSn(callLetter, cmdId);
		ServerToUnitCommand cache = server.getServerToUnitCommandCache().getAndRemoveCommand(sn);
		if(cache != null) {
			Command usercmd = cache.getUserCommand();
			
			String shour = strBody.substring(14, 16);
			String sminus = strBody.substring(16, 18);
			String sseconds = strBody.substring(18, 20);
