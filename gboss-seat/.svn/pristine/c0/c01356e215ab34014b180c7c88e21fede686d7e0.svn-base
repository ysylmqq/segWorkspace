...
!function( $ ) {
	...
	var Datepicker = function(element, options){
		...
		this.formatStr = options.format || "yyyy-MM-dd HH:mm:ss";
	...
	Datepicker.prototype = {
		...
		setValue: function() {
			...
			var formated = Util.formatDate(this.date, this.formatStr);
			if (!this.isInput) {
				if (this.component){
		...
		update: function(){
			...
			//	this.format
			//);
			var dateStr = date ? date : this.element.data('date');
			this.date = Util.parseDate(dateStr, this.formatStr);	
		...
		fill: function() {
			...
			while(prevMonth.valueOf() < nextMonth) {
				...
				//	clsName += ' active';
				//}
				if(DPGlobal.dateEquals(prevMonth, this.date)){
					clsName += ' active';
				}
