(function($) {
	...
	var loadSuccess = function() {
		...
		if (editId && editObj) {
			...
			$("#remark", "#preload_add").val(editObj.remark);

		}

	    $("input", document.preload_add).each(function(){
	    	var parentName = $(this).parent().prop("tagName");
	    	if(parentName == "SPAN"){
		    	var title = $(this).parent().prev().text();
		    	if(title!=''&&title!='备注'){
		    		$(this).on("click",function(){
		    			if(updateMsg.charAt(updateMsg.length-1)==">"){
		    				var index = updateMsg.lastIndexOf(';')==-1?0:(updateMsg.lastIndexOf(';')+1);
		    				updateMsg = updateMsg.substring(0, index);
		    			}
		    			var txt = $(this).val()==""?"空":$(this).val();
		    			updateMsg = updateMsg + title + txt + '->';
					});
		    		$(this).on('change',function(){
		    			if(updateMsg.charAt(updateMsg.length-1)==">"){
		    				updateMsg = updateMsg + $(this).val() + ';';
		    			}
					});
		    	}
	    	}
	...
	var winClose_ubi_import = function() {// 关闭添加页面
		$(document).sgWindow('close', {
			id : 'ubi_import_sim'
		});
	}
	...
	var save = function() {
		...
		if(params.telco==3){//电信14位
			if(params.imei.length != 14){
				$(document).sgPup({message:'message_info',text: "请输入长度为14位的IMEI号！"});
				return false;
			}
		}else{//移动联通15位
			if(params.imei.length != 15){
				$(document).sgPup({message:'message_info',text: "请输入长度为15位的IMEI号！"});
				return false;
			}
		}
		...
		var data = {};
		if(sim_id){
			var remark = "SIM卡资料变更:"+updateMsg+"<br>填写备注:"+$("#remark").val();
			data.upMsg = remark;
			data.preload = params;
		}else{
			data = params;
...
//						$(document).sgWindow(defaults);
	...
	var importItem = function() {// 添加服务项
		var defaults = {
			...
			/*buttons : [ 
			            ...
			            {name: '确定', type: 'button', onpress : subImportNew},
	...
	var batch_update  = function() {// 批量更改
		var defaults = {
			title : '批量更改',
			id : 'batch_update',
			form : 'batch_update',
			url : 'batch_update.html',
			width : 700,
			height : 500,
			/*buttons : [ 
			            //{name: '确定', type: 'submit', onpress : subImport},
	 ...
	 * 新增导入功能
	 */
	var ubi_importItem = function() {
		var defaults = {
			title : 'UBI导入',
			id : 'ubi_import_sim',
			form : 'ubi_sim_import_form',
			url : 'ubi_sim_import.html',
			width : 300,
			height : 140,
			buttons : [ 
			            {name: '确定', type: 'submit', onpress : subUbiImport},
			            {name : '关闭',onpress : winClose_ubi_import}
			          ]
		};
		$(document).sgWindow(defaults);
	}
...
//					$(document).sgWindow(defaults);
 	...
 	var defaults = {
 		        ...
 		        colid:'id',  //主键
 		        colModel : [
 		                {display: '呼号', name : 'call_letter', width : 100, sortable : false},
						{display: 'IMEI/MEID', name : 'imei', width : 150, sortable : false},
						{display: '运营商', name : 'telco', width : 50, sortable : true,formatter:function(value,row){
    	  	                if(value==1){
    	  	                    value = '移动';
    	  	                }else if(value==2){
    	  	                    value = '联通';
						...
						{display: 'TBOX条码', name : 'barcode', width : 100, sortable : false},
						{display: '车架号', name : 'vin', width : 100, sortable : false},
						{display: '车辆颜色', name : 'color', width : 80, sortable : false},
						{display: '是否入网', name : 'unit_id', width : 70, sortable : true,formatter:function(value,row){
	  	  	                if(value==0){
	  	  	                    value = '否';
	  	  	                }else {
	  	  	                    value = '是';
						...
						{display: 'SIM卡状态', name : 'flag', width : 80, sortable : true,formatter:function(value,row){
    	  	                if(value==0){
    	  	                    value = '未激活';
    	  	                }else if(value==1){
    	  	                    value = '激活';
    	  	          ...
    	  	          {display: '套餐状态', name : 'combo_status', width : 80, sortable : true,formatter:function(value,row){
  	  	                if(value==0){
  	  	                    value = '未生效';
  	  	                }else if(value==1){
  	  	                    value = '生效';
						...
						{display: '服务开始时间(运营商)', name : 's_date', width : 150, sortable : false},
						{display: '服务结束时间(运营商)', name : 'e_date', width : 150, sortable : false},
						{display: '扫描时间', name : 'scan_time', width : 150, sortable : false},
  	  	            	{display: '操作日期', name : 'stamp', width : 150, sortable : false}
 		        ],
 		       buttons : [ 
 				 ...
 				 {name: 'UBI导入', bclass: 'import', onpress : ubi_importItem},
 				 {name: '批量更改', bclass: 'import', onpress : batch_update}
	             ],
 		        searchitems :[
 		                      {display:'呼号',html:'<span class="ltext"><input type="text" name="call_letter" class="form-control"/></span>'},
 		                      {display:'IMEI/MEID',html:'<span class="ltext"><input type="text" name="imei" class="form-control"/></span>'},
 		                      {display:'车架号',html:'<span class="ltext"><input type="text" name="vin" class="form-control"/></span>'},
 		                      {display:'TBOX条码',html:'<span class="ltext"><input type="text" name="barcode" class="form-control"/></span>'},
 		                      {display:'运营商',html:'<span class="ltext"><select class="form-control" name="telco"><option value="" selected>请选择...</option><option value="1">移动</option><option value="2">联通</option><option value="3">电信</option><option value="4">其它</option></select></span>'},
 		                      {display:'SIM卡状态',html:'<span class="ltext"><select class="form-control" name="flag"><option value="" selected>请选择...</option><option value="1">激活</option><option value="2">停机</option><option value="3">销号</option><option value="4">停机保号</option><option value="5">服务关停</option><option value="9">删除</option></select></span>'},
 		                      {display:'套餐状态',html:'<span class="ltext"><select class="form-control" name="combo_status"><option value="" selected>请选择...</option><option value="0">未生效</option><option value="1">生效</option><option value="2">变更申请</option><option value="3">变更失败</option></select></span>'}
 		    		        ],
 		    		        exporturl:'../../../preload/exportExcelSim',
 		      		     	order:true,
 		      		        sortname: "stamp",
 		      		        sortorder: "desc"
 		    };
