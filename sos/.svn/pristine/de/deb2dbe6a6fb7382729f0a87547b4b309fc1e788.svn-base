...
public class UnitDaoImpl extends BaseDaoImpl implements UnitDao {
	...
	public List<HashMap<String, Object>> findUnitInNets(
		...
		sb.append(" left join (SELECT GROUP_CONCAT(lm.phone separator ',') linkmanPhone, lm.customer_id from t_ba_linkman lm GROUP BY lm.customer_id) lk on c.customer_id=lk.customer_id");
