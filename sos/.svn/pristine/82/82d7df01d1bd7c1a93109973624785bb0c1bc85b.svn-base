...
@Transactional
public class RenewServiceImpl extends BaseServiceImpl implements RenewService {
	...
	@Override
	public Page<HashMap<String, Object>> findHmTBoxRenewBypage(
			PageSelect<Map<String, Object>> pageSelect) throws SystemException {
		List<HashMap<String, Object>> dataList=renewDao.findHmTBoxRenew(pageSelect.getFilter(), pageSelect.getOrder(), pageSelect.getIs_desc(), pageSelect.getPageNo(), pageSelect.getPageSize());
		int total = renewDao.countHmTBoxRenew(pageSelect.getFilter());
		return PageUtil.getPage(total, pageSelect.getPageNo(), dataList, pageSelect.getPageSize());
	}

	@Override
	public List<HashMap<String, Object>> findAllHmTBoxRenew(
			Map<String, Object> conditionMap) throws SystemException {
		return renewDao.findHmTBoxRenew(conditionMap,null,false,0,0);
	}

	@Override
	public List<HashMap<String, Object>> findTBoxServerExpire(
			Map<String, Object> conditionMap) throws SystemException {
		return renewDao.findTBoxServerExpire(conditionMap,null,false,0,0);
	}
	...
	public Map<String, Object> importHMTBOX(List<String[]> dataList, Long compannyId, Long userId, String companyName, String companyCode)
		...
		try {
			...
			for (int i = 0; i < total; i++) {
				...
				try {
					// 保存客户信息到历史表
					RenewHistory c = new RenewHistory();
					...
					c.setVin(vin);
					// 保存客户信息到t_fee_hm_renew表
					Renew r = new Renew();
					r.setCustomer_name(customer_name);
					r.setBarCode(barCode);
					r.setCall_letter(call_letter);
					r.setCombo_change_date(isCombo_change_date);
					r.setCombo_type(combo_type);
					r.setEngine_no(engine_no);
					r.setEquip_code(equip_code);
					r.setPlate_no(plate_no);
					r.setService_end_date(isService_end_date);
					r.setService_end_newdate(isService_end_newdate);
					r.setService_start_date(isService_start_date);
					r.setTelphone(telPhone);
					r.setVin(vin);
					//将数据存入历史表
					Integer cId = renewDao.addHistory(c);
					//查询要导入的数据vin码是否已经存在
					int vinCount = renewDao.vinIsExist(vin);
					//将数据存入t_fee_hm_renew表,vin存在则更新,不存在则插入
					if(vinCount>0){
						renewDao.updateFeeRenew(customer_name, barCode,call_letter,isCombo_change_date,combo_type,engine_no,equip_code,plate_no,isService_end_date,isService_end_newdate,isService_start_date,telPhone,vin);
					}else{
						renewDao.add(r);
					}
					//根据Vin修改t_fee_info服务截止时间
					if(isService_end_newdate!=null){//转换成data失败说明数据有问题,不进行更新
						renewDao.updateFeeSedateByVin(vin ,service_end_newdate);
					}
					sussNum++;
				} catch (Exception e) {
	...
	public Page<Map<String, Object>> findTBOXByPage(PageSelect<Map<String, Object>> pageSelect) throws SystemException {
		int total = renewDao.countTBOXPage(pageSelect.getFilter());
		List<Map<String, Object>> list = renewDao.findTBOXByPage(pageSelect.getFilter(), pageSelect.getOrder(),
	...
	@Override
	public Page<HashMap<String, Object>> findHistoryRenew(
			PageSelect<Map<String, Object>> pageSelect,String vin) throws SystemException {
		List<HashMap<String, Object>> dataList=renewDao.findHistoryRenew(pageSelect.getFilter(), pageSelect.getOrder(), pageSelect.getIs_desc(), pageSelect.getPageNo(), pageSelect.getPageSize(),vin);
		int total = renewDao.countHistoryRenew(pageSelect.getFilter(),vin);
		return PageUtil.getPage(total, pageSelect.getPageNo(), dataList, pageSelect.getPageSize());
	}
}
