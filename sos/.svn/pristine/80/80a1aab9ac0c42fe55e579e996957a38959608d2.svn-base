...
public class VehicleDaoImpl extends BaseDaoImpl implements VehicleDao {
	...
	public List<HashMap<String, Object>> findServiceFeePage(Long companyno,
	 ...
	 private StringBuffer getConditionFee(StringBuffer sb,Map<String, Object> conditionMap, Long commpanyNo){
	 	sb.append(" FROM( SELECT v.vehicle_id,v.plate_no, c.subco_code,c.customer_name,u.call_letter,u.fix_time,u.sales FROM  ");
		...
		sb.append(" where 1=1");
		if (Utils.isNotNullOrEmpty(conditionMap)){
			String queryType = Utils.isNullOrEmpty(conditionMap.get("fee"))? "" : conditionMap.get("fee").toString();
			if("1".equals(queryType)){
				sb.append(" and b.month_fee  < 40 ");
			...
			if(!"0".equals(companyCode) && StringUtils.isNotBlank(companyCode)){
				sb.append(" and a.subco_code='").append(companyCode).append("'");
			}
	...
	@Override
	public Integer updateVehicleStataus(Map<String, Object> param, int status)
			throws SystemException {
		StringBuffer sb = new StringBuffer();
		sb.append(" update t_ba_vehicle set ");
		sb.append(" vehicle_status="+status+"");
		sb.append(" where vehicle_id="+param.get("vehicle_id")+"");
		SQLQuery query=sessionFactory.getCurrentSession().createSQLQuery(sb.toString());
		return query.executeUpdate();
	}

	@Override
	public Integer updateVehiclePwd(Long vehicleId, String servicePwd,
			String privatePwd, String updateType) throws SystemException {
		StringBuffer sb=new StringBuffer();
		sb.append(" update t_ba_vehicle set ");
		...
		sb.append(" where vehicle_id=").append(vehicleId);
		Query query = sessionFactory.getCurrentSession().createSQLQuery(sb.toString());
		return query.executeUpdate();
	}

	@Override
	public boolean isVehicleepeat(Long vehicleId, String plateNo, String vin,
			String engineNo) throws SystemException {
		Criteria criteria = sessionFactory.getCurrentSession().createCriteria(
				Vehicle.class);
		int count = 0;
		if (vehicleId != null) {
		...
		if (StringUtils.isNotBlank(plateNo)) {	//车牌号码
			criteria.add(Restrictions.eq("plate_no", plateNo));
		}
		if (StringUtils.isNotBlank(vin)) {	//车架号
			criteria.add(Restrictions.eq("vin", vin));
		}
		if (StringUtils.isNotBlank(engineNo)) {	//发动机号码
			criteria.add(Restrictions.eq("engine_no", engineNo));
		}
		count = criteria.list().size();
		if (count > 0) {
			return true;
