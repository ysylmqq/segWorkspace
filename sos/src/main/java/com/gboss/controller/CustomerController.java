...
public class CustomerController extends BaseController{
	...
	public @ResponseBody HashMap add(@RequestBody Customer customer, BindingResult bindingResult,HttpServletRequest request) throws SystemException {
		...
		if (post.equals(request.getMethod())) {
			...
			//将参数添加到日志中
			request.setAttribute(SystemConst.OPLOG_PARAMS_KEY, JsonUtil.oToJ(customer,true));
	...
	public @ResponseBody HashMap del(@RequestBody Customer customer, BindingResult bindingResult,HttpServletRequest request) throws SystemException {
	...
	public @ResponseBody HashMap update(@RequestBody Customer customer, BindingResult bindingResult,HttpServletRequest request) throws SystemException {
	...
	public @ResponseBody HashMap file(@RequestBody Customer customer, BindingResult bindingResult,HttpServletRequest request) throws SystemException{
		String userId = (String)request.getSession().getAttribute(SystemConst.ACCOUNT_ID);
		HashMap<String, Object> resultMap=new HashMap<String, Object>();
	...
	public @ResponseBody HashMap getFileNo(@RequestBody Customer customer, BindingResult bindingResult,HttpServletRequest request) throws SystemException{
		HashMap<String, Object> resultMap=new HashMap<String, Object>();
	...
	public @ResponseBody String getCustomerPhone(Long id, HttpServletRequest request) throws SystemException {
	...
	public @ResponseBody HashMap<String, Object> getDetailMsg(Long id, HttpServletRequest request) throws SystemException {
		if (LOGGER.isDebugEnabled()) {
	...
	public @ResponseBody HashMap<String, Object> getDetailMsgBycl(String call_letter, HttpServletRequest request) throws SystemException {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("获得客户车辆详细信息开始");
		}
		HashMap<String, Object> results = null;
   ...
   *//*
	@RequestMapping(value = "/customer/getDetailMsg")
	public @ResponseBody HashMap<String, Object> getCustomerInfo(HttpServletRequest request) throws SystemException {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("获得客户信息开始");
		}
		HashMap<String, Object> results = null;
		try {
			String userId = (String)request.getSession().getAttribute(SystemConst.CUST_INFO);
			results= customerService.getDetailMsg(0L);
		} catch (Exception e) {
			LOGGER.error(e.getMessage(), e);
			e.printStackTrace();
			// 同时把异常抛到前台
	...
	*/
	...
	@RequestMapping(value = "/customer/lockLargeCustomer")
	public @ResponseBody HashMap<String, Object> lockLargeCustomer(Long id, HttpServletRequest request) throws SystemException {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("分公司加锁解锁开始");
		}
		boolean flag = true;
		String msg = SystemConst.OP_SUCCESS;
		HashMap<String, Object> results = new HashMap<String, Object>();
		try {
			Customer cust = customerService.get(Customer.class, id);
			Long subco_no = cust.getSubco_no();
			LargeCustLock custLock = customerService.get(LargeCustLock.class, id);
			if(null == custLock){
				LargeCustLock cust_lock = new LargeCustLock();
				cust_lock.setCustomer_id(id);
				cust_lock.setLocktag(1);
				cust_lock.setSubco_no(subco_no);
				customerService.save(cust_lock);
			}else{
				Integer locktag = custLock.getLocktag();
				locktag = locktag == 0 ? 1:0;
				custLock.setLocktag(locktag);
				customerService.update(custLock);
			}
		} catch (Exception e) {
			flag = false;
		...
		results.put("msg", msg);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("分公司加锁解锁结束");
		}
		return results;
	}
	
	
	@RequestMapping(value = "/customer/getTotalFeeMsg")
	public @ResponseBody HashMap<String, Object> getTotalFeeMsg(Long cust_id, HttpServletRequest request) throws SystemException {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("获得集客每月缴费开始");
		}
		HashMap<String, Object> results = null;
		try {
			//计算自动加锁
			LargeCustLock lock = custVehicleService.get(LargeCustLock.class, cust_id);
			String locktime = null;
			if(null != lock ){
				    lock.setLocktag(1);
					locktime = DateUtil.format(lock.getStamp(), DateUtil.YMD_DASH_WITH_FULLTIME);
				custVehicleService.update(lock);
			}else{
				LargeCustLock lock_1 = new LargeCustLock();
				Customer cust = custVehicleService.get(Customer.class, cust_id);
				lock_1.setCustomer_id(cust_id);
				lock_1.setLocktag(1);
				lock_1.setSubco_no(cust.getSubco_no());
				custVehicleService.save(lock_1);
			}
			
			results= custVehicleService.getTotalFeeMsg(cust_id, locktime);
		} catch (Exception e) {
			LOGGER.error(e.getMessage(), e);
			e.printStackTrace();
			// 同时把异常抛到前台
