...
//	@LogOperation(description = "修改SIM卡", type = SystemConst.OPERATELOG_UPDATE,model_id=200101)
	 ...
	 */
	@RequestMapping(value = "/preload/batchUpdateSimFlag",method = RequestMethod.POST)
	@LogOperation(description = "批量跟新sim卡状态", type = SystemConst.OPERATELOG_ADD, model_id =20101)
	public @ResponseBody Map<String, Object> batchUpdateSimFlag(@RequestParam("sim_file") MultipartFile sim_file,
			@RequestParam(required = false) Boolean isOverride,Integer flag,
			HttpServletRequest request, HttpServletResponse response)
			throws SystemException, IOException, IllegalAccessException, InvocationTargetException {
		Long compannyId = (long) 201;  // 只有海马
		String uid = (String) request.getSession().getAttribute(
				SystemConst.ACCOUNT_ID);
		Long userId = Long.valueOf(uid);
		Map<String, Object> map = new HashMap<String, Object>();
		...
		if (size > 10000000) {
			...
			FileUtils.copyInputStreamToFile(sim_file.getInputStream(), descFile);
			List<String[]> dataList = CreateExcel_PDF_CSV.getData(descFile);
			map = preloadService.batchUpdateSimFlag(dataList, compannyId, userId, flag);
			//使用多线程进行更新
			//map = preloadService.batchUpdateSimFlagThread(dataList, compannyId, userId, flag);
			descFile.delete();
		}
		return map;
	}
	 ...
	 * @throws SystemException
	 */
	@RequestMapping(value = "/preload/importSimNew",method = RequestMethod.POST)
	@LogOperation(description = "sim卡资料导入", type = SystemConst.OPERATELOG_ADD, model_id =20100)
	public @ResponseBody Map<String, Object> importSimNew(@RequestParam("sim_file") MultipartFile sim_file,
		...
		if (size > 10000000) {
			...
			// 解析并保存用户数据   compannyId  用户类型  70792
			//map = preloadService.importSim(dataList, compannyId, userId, type, telco);
			map = preloadService.importSimNew(dataList, compannyId, userId, type, telcoV);
			/*boolean remark = CreateExcel_PDF_CSV.checkExcel(descFile, type);
			if(remark){
				List<String[]> dataList = CreateExcel_PDF_CSV.getData(descFile);
				// 解析并保存用户数据
...
//			}
	...
	@RequestMapping(value = "/preload/exportExcelErrorSim",method = RequestMethod.POST)
	@LogOperation(description = "SIM导入时，导入失败的记录", type = SystemConst.OPERATELOG_SEARCHKEY, model_id =20061)
	public @ResponseBody
	void exportExcelErrorSim(@RequestBody Preload test,BindingResult bindingResult,
			HttpServletRequest request, HttpServletResponse response)
			 {
		try {
			String compannyId = (String) request.getSession().getAttribute(
			...
			String[][] title = {{"序号","10"},{"ICCID","20"},{"IMSI","20"},{"AKEY","20"},{"ESN","20"},
					{"EVDO账号","30"},{"EVDO密码","20"},{"设备号","20"},{"开通日期","20"},
					{"TBOX条码","20"},{"IMEI/MEID","20"},{"备注","30"}
			...
			String[] values = null;
			 ...
			 List< Preload >  preload  =new ArrayList<Preload>();
			 preload.add(test);
			for(int i=0; i< preload.size(); i++){
				values = new String[title.length];
				Preload upgrade = preload.get(i);
				values[0] = String.valueOf(i+1);
				values[1] = upgrade.getIccid()==null? "":upgrade.getIccid();
				values[2] = upgrade.getImsi() ==null?"":upgrade.getImsi() ;
				values[3] = upgrade.getAkey() ==null?"":upgrade.getAkey();
				values[4] = upgrade.getEsn()==null?"":upgrade.getEsn();
				values[5] = upgrade.getW_user()==null?"":upgrade.getW_user();
				values[6] = upgrade.getW_password()==null?"":upgrade.getW_password();				
				values[7] = upgrade.getCall_letter()==null ?"":upgrade.getCall_letter();
				values[8] = upgrade.getS_date() == null?"":upgrade.getS_date().toString();
				
				/*Integer comboId =  upgrade.getCombo_id()==null ?null:Integer.valueOf(upgrade.getCombo_id().toString());
				if(comboId != null && comboId ==0){
					values[9] = "低配";
				}else if(comboId != null && comboId ==1){
					values[9] = "海马测试";
				}else if(comboId != null && comboId ==2){
					values[9] = "10元/月";
				}else if(comboId != null && comboId ==6){
					values[9] = "海马高配";
				}else if(comboId != null && comboId ==7){
					values[9] = "海马低配";
				}else if(comboId != null && comboId ==6){
					values[9] = "高配";
				}else{
				...
				values[9] = upgrade.getBarcode()==null ?"":upgrade.getBarcode();
				values[10] = upgrade.getImei()==null ?"":upgrade.getImei();
				
				/*Integer telco =  upgrade.getTelco()==null ?null:Integer.valueOf(upgrade.getTelco().toString());
				...
				values[11] = upgrade.getRemark()==null ?"":upgrade.getRemark();
				valueList.add(values);
			}
			//获得分公司中英文名称
			OpenLdap openLdap = OpenLdapManager.getInstance();
			CommonCompany commonCompany =  openLdap.getCompanyById(compannyId);
			CreateExcel_PDF_CSV.createExcel(valueList, response, "SIM卡导入失败记录", title,commonCompany.getCnfullname(),commonCompany.getEnfullname(),false);
		} catch (Exception e) {
			LOGGER.error(e.getMessage(), e);
			// 打印
			e.printStackTrace();
