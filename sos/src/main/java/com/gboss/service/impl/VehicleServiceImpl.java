...
public class VehicleServiceImpl extends BaseServiceImpl implements VehicleService {
	...
	public Page<HashMap<String, Object>> findVehiclePage(
		...
		int total=vehicleDao.countVehicleInfo(pageSelect.getFilter(), companyno);
		List<HashMap<String, Object>> list=vehicleDao.getVehicleInfo(pageSelect.getFilter(), companyno, pageSelect.getOrder(), pageSelect.getIs_desc(),pageSelect.getPageNo(),pageSelect.getPageSize());
	...
	public List<HashMap<String, Object>> getVehicleTree(Customer customer) {
		...
		List<Vehicle> list = vehicleDao.getVehiclesByCustid(customer.getCustomer_id());
		for(Vehicle vehicle:list){
			HashMap<String, Object> child = new HashMap<String, Object>();
			Unit unit = unitDao.getByCustAndVehicle(customer.getCustomer_id(), vehicle.getVehicle_id());
			Servicetime stime = servicetimeDao.getByUnitid(unit.getUnit_id());
			child.put("id", vehicle.getVehicle_id());
			String edate = "";
			//这里的pay_model指的是需要显示哪种服务截止时间
			if(stime!=null && stime.getService_edate()!=null && customer.getPay_model()==1){
				edate = DateUtil.format(stime.getService_edate(), DateUtil.YMD_DASH);
				child.put("name", vehicle.getPlate_no()+"(服务截止时间:"+edate+")");
			}else if(stime!=null && stime.getSim_edate()!=null && customer.getPay_model()==2){
				edate = DateUtil.format(stime.getSim_edate(), DateUtil.YMD_DASH);
	...
	public Page<HashMap<String, Object>> findServiceFeePage(Long companyno,
		...
		int total=vehicleDao.countServiceFee(companyno, pageSelect.getFilter());
		List<HashMap<String, Object>> list=vehicleDao.findServiceFeePage(companyno,pageSelect.getFilter(), pageSelect.getOrder(), pageSelect.getIs_desc(),pageSelect.getPageNo(),pageSelect.getPageSize());
		return PageUtil.getPage(total, pageSelect.getPageNo(), list, pageSelect.getPageSize());
	}
	...
	public Page<HashMap<String, Object>> findOnlineNoFeePage(Long companyno,
			PageSelect<Map<String, Object>> pageSelect) throws SystemException {
		int total=vehicleDao.countOnlineNoFee(companyno, pageSelect.getFilter());
		List<HashMap<String, Object>> list=vehicleDao.findOnlineNoFeePage(companyno,pageSelect.getFilter(), pageSelect.getOrder(), pageSelect.getIs_desc(),pageSelect.getPageNo(),pageSelect.getPageSize());
		return PageUtil.getPage(total, pageSelect.getPageNo(), list, pageSelect.getPageSize());
	}
	...
	public List<HashMap<String, Object>> findOnlineNoFeeList(Long companyno,
			Map map) throws SystemException {
		return vehicleDao.findOnlineNoFeePage(companyno,map, null, false, 0,0);
	}
