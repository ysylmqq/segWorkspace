...
if (typeof SEGUtil_ws === "undefined") { throw new Error("segseatlib requires util"); };
...
var segseatlib = {
	...
	*/
	...
	//客户端定义
	jsclient: function(_id, _hosts, _username, _password, _seatid) {
		segseatlib.init();
		...
		var login = function() {
			...
			var callletterlist = txtcallletters.value.split(";");
			var loginobj = new loginbuilder(username, password, callletterlist, seatid);
		...
		//坐席向服务器请求挂警（暂时不处理这个警情），可以接收其他警情
		this.CancelPauseAlarm = function(seatname, callletter, alarmsn, busy) {
			...
			if (busy != null) {
				tmpobj = new tmpbuilder(seatname, callletter, alarmsn, busy);
			}
		...
		*************************************************************************************/
		this.AskAlarmList = function(seatname, onlyself) {
			...
			var tmpobj = new tmpbuilder(seatname, onlyself);
		...
		var processDeliverAlarm = function(content) {
			try {
				...
				if (typeof(ondeliveralarm) != "undefined") {
					ondeliveralarm(selfclient, deliver.gpsinfo, deliver.gatewayid, deliver.gatewaytype);
				}
		    } catch (err) {
		...
		var processAskAlarmListACK = function(content) {
			try {
				...
				if (typeof(onaskalarmlistack) != "undefined") {
					onaskalarmlistack(selfclient, pkg.retcode, pkg.retmsg, pkg.alarms);
				};
		    } catch (err) {
