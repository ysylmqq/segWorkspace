...
public class LeopaardDownloadCmdEncoder implements OutputPackageEncoder {
	...
	public ServerToUnitCommand encode(CommandBuff.Command userCmd, UnitSocketSession unitSession) throws Exception {
		...
		case CmdUtil.CMD_ID_LEOPAARD_OPEN_SUNROOF:
			return getSunroofCmd(userCmd, unitSession, true);
		case CmdUtil.CMD_ID_LEOPAARD_CLOSE_SUNROOF:
			return getSunroofCmd(userCmd, unitSession, false);
		...
		case CmdUtil.CMD_ID_BOX_CLOSE_AIR_CONDITIONING:
		...
		case CmdUtil.CMD_ID_LEOPAARD_FAULT_DIAGNOSIS:
			return getFaultDiagnosisCmd(userCmd, unitSession);
		default:
	...
	private ServerToUnitCommand getSunroofCmd(Command userCmd, UnitSocketSession unitSession, boolean isOpen)
			throws Exception {
		byte param = (byte) (isOpen ? 1 : 2);
		byte[] cmdParam = new byte[2];
		cmdParam[0] = 0x04;
	...
	private ServerToUnitCommand getFaultDiagnosisCmd(Command userCmd, UnitSocketSession unitSession)
			throws Exception {
		byte[] cmdParam = new byte[2];
		cmdParam[0] = 0x06;
		cmdParam[1] = 0x01;
