...
public class HMStatus {
	...
	public static HMStatus parse(byte[] data, int offset){
		HMStatus status = new HMStatus();
	...
	public static void main(String[] args) {
		...
		HMStatus status = HMStatus.parse(data, 0);
	 ...
	 * 报警状态1
	 */
	private static void parseStatus1(HMStatus status, byte flag){
	 ...
	 * 报警状态2
	 */
	private static void parseStatus2(HMStatus status, byte flag){
		if((flag & STATUS_AND[4]) != 0){
			//碰撞报警*
			status.addStatus(12);
			status.setToSpecialAlarm();
	 ...
	 * 故障状态
	 */
	private static void parseStatus3(HMStatus status, byte flag){
		if((flag & STATUS_AND[7]) != 0){
			//碰撞检测故障
			status.addStatus(46);
		}
...
//		}
	 ...
	 * 车灯状态
	 */
	private static void parseStatus4(HMStatus status, byte flag){
		if((flag & STATUS_AND[7]) != 0){
			//远光灯
			status.addStatus(42);
		}else{
	 ...
	 * 车门状态
	 */
	private static void parseStatus5(HMStatus status, byte flag){
		if((flag & STATUS_AND[7]) != 0){
			//左前门（司机）
	 ...
	 * 档位
	 */
	private static void parseStatus6(HMStatus status, byte flag){
		if((flag & STATUS_AND[3]) != 0){
			//P
	 ...
	 * 其他
	 */
	private static void parseStatus7(HMStatus status, byte flag){
		if((flag & STATUS_AND[7]) != 0){
			//刹车
			status.addStatus(39);
		}
