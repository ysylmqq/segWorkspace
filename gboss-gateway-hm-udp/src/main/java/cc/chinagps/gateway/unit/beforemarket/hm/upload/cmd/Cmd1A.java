...
public class Cmd1A extends CheckLoginHandler{
	...
	public boolean handlerPackageSessionChecked(BeforeMarketPackage ppkg,
		...
		if(sumCmdId == 0x11){
			...
			int unit_ack_result = Util.getShort(body, 1) & 0xFFFF;
			HMUploadUtil.commonResponseUseHeadSN(ppkg, server, session, unit_ack_result, 0x00A5);
			return true;
...
//			if(success == HMConstants.HM_STATUS_SUCCESS){
//				String version = APlanUtil.getCString(body, 3, 4);
//				HMUploadUtil.handleVersion(server, session.getUnitInfo().getCallLetter(), version);
//			}
