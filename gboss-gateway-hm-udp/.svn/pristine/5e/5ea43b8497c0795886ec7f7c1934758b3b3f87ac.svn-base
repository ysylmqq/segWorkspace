...
public class KaiyiTravelInfo {
	//数据长度(字节)
	private int dataLength;
	
	//起点GPS信息
	private KaiyiGPSInfo startGPS;
	...
	public KaiyiGPSInfo getStartGPS() {
		return startGPS;
	}

	public void setStartGPS(KaiyiGPSInfo startGPS) {
	...
	public static KaiyiTravelInfo parse(byte[] data, int offset) throws Exception{
		KaiyiTravelInfo travelInfo = new KaiyiTravelInfo();
		//前2字节为长度
		int position = offset;
		short dataLength = Util.getShort(data, position);
		position += 2;
		
		KaiyiGPSInfo startGPS = KaiyiGPSInfo.parse(data, position);
	...
	public static void main(String[] args) throws Exception {
		...
		KaiyiGPSInfo gps = new KaiyiGPSInfo();
		gps.setCourse(50);
		...
		KaiyiTravelInfo t = new KaiyiTravelInfo();
		t.setAverageOil(10);
		...
		byte[] data = t.encode();
		KaiyiTravelInfo pt = KaiyiTravelInfo.parse(data, 0);
