(function($){
            			  ...
            			  if(manager){
            				  if(data.items.length>0){
            					  ...
            					  $.each(data.items,function(k,v){
                					  $("#vinList").append('<li name='+v.vehicle_id+' id="rec_'+k+'">'+v.vin+'</li>');
                				  	 }); 
                				  $("#vinList").show();
            				  }
            				  if(data.items.length==0){
            					  $("#vinList").append('<li>没有数据！</li>');
            					  $("#vinList").show();
            					  document.nw_private_form.reset();
            				  }
            				  if(data.items.length==1){
            					  var item = data.items[0];
            					  select_word(item.vin, item.vehicle_id);
            					  $("#vinList").hide();
            				  }
            				}
            		  }else{
        ...
        function next_word(idx){
            $("#vinList li").removeClass("current");
        ...
        $("#vin").keyup(function(event){
            //方式一：按回车时出现推荐栏
           if(!$("#vinList li.current").text() && event.keyCode==13){
        ...
        //var idx = -1; //初始化高亮项
        $("#vin").keyup(function(event){
            max = $('#vinList li').length - 1 //一共有max个推荐词(因为从0开始计数，所以要-1)
            if(event.keyCode == 40){ //按了下箭头
                ...
                if($("#vinList li.current").text()){ //如果此时有高亮候选词，选中它并搜索
                    var id = $('#vinList li.current').attr('name');	
                    select_word($("#vinList li.current").text(),id);
                }
            }
        });

        //鼠标点击推荐词被点击时
        $("body").on("click", "#vinList li", function(){
        	var id = $('#vinList li.current').attr('name');	
            select_word($(this).text(),id);
        });
        //鼠标滑过推荐词时
        $("body").on("mouseover", "#vinList li", function(){ 
