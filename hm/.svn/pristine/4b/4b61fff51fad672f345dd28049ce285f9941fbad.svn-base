(function($) {	
    ...
    function outputSuccessCmd(){
   	 var d = dialog({
            title: '上传批次',
            content: '<input autofocus id="batchType" style="border-radius: 3px;height:30px;border:1px solid #4791d2" placeholder="输入上传批次(必填)"  >',
            okValue: '导出',
            height:10,
            lock:true,
            ok: function() {
           	 ...
           	 /*if(batchType == ""){
                    ...
                    return false;
           	    	...
           	    	window.location.href="../../../fccmdManage/exportExcelFailCmd?batchSeq="+batchType;
                  	    return true;
           	 }*/
           	 window.location.href="../../../fccmdManage/exportExcelFailCmd?batchSeq="+batchType+"&type=1";
           	 return true;
            },
            cancelValue: '取消',
            cancel: function() {
   	 ...
   	 d.showModal();
   	 $("table td.ui-dialog-body").css({"padding-top":"5px"});
   	 $("#batchType").parent().css({"height":"30px"});
   }
    
    function outputFailCmd(){
    	 var d = dialog({
             title: '上传批次',
             content: '<input autofocus id="batchType" style="border-radius: 3px;height:30px;border:1px solid #4791d2" placeholder="输入上传批次(必填)"  >',
             okValue: '导出',
             ...
             ok: function() {
            	 ...
            	 window.location.href="../../../fccmdManage/exportExcelFailCmd?batchSeq="+batchType+"&type=0";
            	 return true;
             },
             cancelValue: '取消',
             cancel: function() {
	...
	var batchOperat = function() {// 添加服务项
		var defaults = {
			title : '批量操作',
			id : 'import_sim',
 	...
 	var defaults = {
				...
				url:'../../../fccmdManage/getAllSimPage',
 		        ...
 		        colModel : [
						...
						{display: '省流量指令状态', name : 'currSaveflowStatus', width : '15%',align:'center', sortable : false,formatter:function(value,row){
							   var statusStr = '其他';
 		       ...
 		       buttons : [
 		    	  {name: '省流量模式',bclass : 'open',onpress : openItem},
 		    	 /* {name: '取消开启指令',bclass : 'close',onpress : cancleOpenItem},*/
			      {name: '普通模式',bclass : 'close',onpress : closeItem},
			      /*{name: '取消关闭指令',bclass : 'close',onpress : cancleCloseItem},*/
			      {name: '批量操作',bclass : 'close',onpress : batchOperat},
			      {name: '导出执行失败指令',bclass : 'close',onpress : outputFailCmd},
			      {name: '导出执行成功指令',bclass : 'close',onpress : outputSuccessCmd}
 		       ],
		 	   searchitems : [
				{
				...
				{
					display : '<span style="padding-left:2px;">省流量指令状态</span>',
					name : 'flag',
					html : '<select style="width:80px"  name="currSaveflowStatus"><option value="1">开启</option><option value="0">关闭</option></select>'
				}]
