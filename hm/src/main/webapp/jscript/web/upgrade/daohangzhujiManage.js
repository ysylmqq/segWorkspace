(function($) {	
    ...
    function openItem(){
		...
		if ($('input[type=checkbox][checked=checked]', bDiv) != null
			...
			$(document).sgConfirm({text: '确定开启所选sim卡的导航主机指令吗?',cfn:function(r){ 
			    if(r){
			    	var simIds=[];
	            	...
	            	$.ajax({
	            		  ...
	            		  async: false,   
	            		  dataType : 'json',     
	            		  url : '../../../fccmdManage/sendNaviHostCmd',
	            		  data:JSON.stringify(params),
	            		  contentType : 'application/json',
	            		  success : function(data) {
	            			  console.log(data);
    ...
    function closeItem(){
		...
		if ($('input[type=checkbox][checked=checked]', bDiv) != null
			...
			$(document).sgConfirm({text: '确定关闭所选sim卡的导航主机指令吗?',cfn:function(r){ 
			    if(r){
			    	var simIds=[];
    ...
    function cancleOpenItem(){
		...
		if ($('input[type=checkbox][checked=checked]', bDiv) != null
			...
			$(document).sgConfirm({text: '确定取消所选sim卡的导航主机开启指令吗?',cfn:function(r){ 
			    if(r){
			    	var simIds=[];
	            	...
	            	$.ajax({
	            		  ...
	            		  async: false,   
	            		  dataType : 'json',     
	            		  url : '../../../fccmdManage/cancleNaviHostCmd',
	            		  data:JSON.stringify(params),
	            		  contentType : 'application/json',
	            		  success : function(data) {
	            			  console.log(data);
    ...
    function cancleCloseItem(){
		...
		if ($('input[type=checkbox][checked=checked]', bDiv) != null
			...
			$(document).sgConfirm({text: '确定取消所选sim卡的导航主机关闭指令吗?',cfn:function(r){ 
			    if(r){
			    	var simIds=[];
 	...
 	var defaults = {
				...
				url:'../../../fccmdManage/getAllSimPage',
 		        ...
 		        colModel : [
						...
						{display: '导航主机当前状态', name : 'currNavihostStatus', width : '15%',align:'center', sortable : false,formatter:function(value,row){
							   var statusStr = '其他';
 		       ...
 		       buttons : [
 		    	  {name: '连接',bclass : 'open',onpress : openItem},
 		    	  /*{name: '取消开启指令',bclass : 'cancle',onpress : cancleOpenItem},*/
			      {name: '断开',bclass : 'close',onpress : closeItem}/*,			      
			      {name: '取消关闭指令',bclass : 'cancle',onpress : cancleCloseItem}*/
 		       ],
		 	   searchitems : [				
				{
				...
				/*{
					display : '<span style="padding-left:2px;">指令下发状态</span>',
					name : 'flag',
					html : '<select style="width:80px"  name="remoteCtrlStatus">'+
						   '<option value="1">初始值</option>'+
						   '<option value="2">要求下发</option>'+
						   '<option value="3">已发查车</option>'+
						   '<option value="4">已发查车</option>'+
						   '<option value="5">指令执行中</option>'+
						   '<option value="6">指令执行失败</option>'+
						   '<option value="7">指令执行成功</option>'+
						   '<option value="8">指令取消</option>'+
						   '</select>'
				},*/				
				{
					display : '<span style="padding-left:2px;">导航主机状态</span>',
					name : 'flag',
					html : '<select style="width:80px"  name="currNavihostStatus">'+						   
