...
public class AlarmDaoImpl extends BaseDaoImpl implements AlarmDao {
	...
	public Page<HashMap<String, Object>> search(PageSelect<Alarm> pageSelect, Long subco_no) {
		...
		System.err.println("provider pageSelect "+pageSelect.toString());
	...
	public List<Map<String, Object>> faultCount(Map<String, Object> map) {
		...
		StringBuffer sbCount1 =new StringBuffer( " SELECT  faulttypeid,COUNT(1) AS value   FROM t_hm_fault_analysis an WHERE 1=1  ");
		StringBuffer sbCount2 =new StringBuffer( " SELECT COUNT(DISTINCT(call_letter)) as count FROM t_hm_fault_analysis an WHERE 1=1  ");
		StringBuffer sb =new StringBuffer( " SELECT faulttypeid , faulttypename as status_name FROM `t_hm_faulttype`  ");

		String start_date = (String) map.get("start_date");
		if (StringUtils.isNotBlank(start_date)) {
			start_date = start_date+" 00:00:00";
			sbCount1.append(" and an.starttime >  '").append(start_date).append("' ");
			sbCount2.append(" and an.starttime >  '").append(start_date).append("' ");
		}
		
		String end_date = (String) map.get("end_date");
		if (StringUtils.isNotBlank(end_date)) {
			end_date = end_date+" 23:59:59";
			sbCount1.append(" and an.starttime < '").append(end_date).append("' ");
			sbCount2.append(" and an.starttime < '").append(end_date).append("' ");
		}
		
		sbCount1.append("  GROUP BY faulttypeid ");
		...
		Query query = mysql1SessionFactory.getCurrentSession().createSQLQuery(sbCount1.toString());  
		query.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP);
		List list1=query.list();
		
		Query query1 = mysql1SessionFactory.getCurrentSession().createSQLQuery(sb.toString());  
		query1.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP);
		List statusList=query1.list();
		Map<Integer,Object> statusMap = new HashMap<Integer,Object>();
		for (int i = 0; i < statusList.size(); i++) {
			Map<String,Object> statusTemp = (Map<String, Object>) statusList.get(i);
			String status_id = (String) (statusTemp.get("faulttypeid")+"");
			statusMap.put(Integer.valueOf((status_id) ), statusTemp.get("status_name"));
		}
		
		for (int i = 0; i < list1.size(); i++) {
			Map<String,Object> mapTemp = (Map<String, Object>) list1.get(i);
			int faulttypeid =Integer.valueOf( (String)(mapTemp.get("faulttypeid")+""));
			String typeName = (String) statusMap.get(faulttypeid);
	...
	public List<HashMap<String, Object>> search() {
		...
		sb.append(" FROM t_hm_alarm_analysis  an LEFT JOIN t_ba_unit unit ON an.call_letter = unit.call_letter ");
		sb.append(" LEFT JOIN t_ba_vehicle veh ON veh.vehicle_id = unit.vehicle_id ");
		sb.append(" LEFT JOIN t_ba_customer cust ON unit.customer_id = cust.customer_id  WHERE 1=1 and veh.subco_no = '201' order by an.analysis_id desc ");
		
		Query query = mysql1SessionFactory.getCurrentSession().createSQLQuery(sbQuery.append(sb).toString());  
		query.setFirstResult(PageUtil.getPageStart(1, 10));
		query.setMaxResults(10);
		query.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP);
		List<HashMap<String, Object>> res = query.list();
		System.err.println("res "+res.size());
		return res;
	}
