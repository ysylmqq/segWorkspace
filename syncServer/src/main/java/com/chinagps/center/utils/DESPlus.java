...
public class DESPlus {
	...
	/**
	 * 密钥为8位，不能多不能少，指定key，则key+DefaultKey后截前8位
	 */
	...
	/**
	 * 密钥小于8位，补足8位，超过8位，截取前8位
	 * 总之密钥为8位字符
	 * @param key
	 */
	public DESPlus(String key) {
		this.strKey = String.valueOf(key+this.strDefaultKey).substring(0, 8);
		return;
	}
	...
	public static String getEncPWD(String password) {
		...
		String miwen = m.Encode(password);
		return miwen;
	}
	...
	public static void main(String args[]) {
		...
		String pwd ="1B2A9116F037173DBB9505E036C6DC83";
		pwd = "密码输入长度错误!";
		//pwd="4DE427FE9F01230C1";
		
		System.out.println(DESPlus.getEncPWD(pwd));
		...
		String minwen = m.Decode(miwen).toString().trim();
		System.out.println("明文:[" + minwen + "]");**/
		
		String s1 = "4DE427FE9F01230C";
		String mingwen = m.Decode(s1).toString().trim();
	...
	public String Decode(String str) {
		...
		byteMiW = this.hexStr2ByteArr(str);

		i2 = this.DECRYPT(this.strKey.getBytes(), byteMingW, byteMiW,
				byteMiW.length);
