...
public class OpenLdap {
    ...
    public void add(Object obj) {
        try {
            ...
            if(obj instanceof CommonCompany){
                ...
                if(Util.isNotBlank(company.getCoordinates())){
                	attrs.put("coordinates", company.getCoordinates());
                }
                if(Util.isNotBlank(company.getPhone())){
                	attrs.put("phone", company.getPhone());
                }
                if(Util.isNotBlank(company.getPhone2())){
                	attrs.put("phone2", company.getPhone2());
                }
                if(Util.isNotBlank(company.getPhone3())){
                	attrs.put("phone3", company.getPhone3());
                }
                if(Util.isNotBlank(company.getMajor())){
                	attrs.put("major", company.getMajor());
                }
                if(Util.isNotBlank(company.getAptitude())){
                	attrs.put("aptitude", company.getAptitude());
                }
                if(Util.isNotBlank(company.getImage())){
                	attrs.put("image", company.getImage());
                }
                if(Util.isNotBlank(company.getImage2())){
                	attrs.put("image2", company.getImage2());
                }
                if(Util.isNotBlank(company.getRemark())){
                	attrs.put("remark", company.getRemark());
                }
                init();
                ...
                if(Util.isNotBlank(operator.getUserPassword())){
                	attrs.put("userPassword", LDAPSecurityUtils.getOpenLDAPSHA(operator.getUserPassword()));
                }
                ...
                if(Util.isNotBlank(operator.getOrgid())){
                	attrs.put("orgid", operator.getOrgid());
                }
                ...
                if(Util.isNotBlank(operator.getNumberplate())){
                	attrs.put("numberplate", operator.getNumberplate());
                }
                if(Util.isNotBlank(operator.getCustomerid())){
                	attrs.put("customerid", operator.getCustomerid());
                }
                if(Util.isNotBlank(operator.getOrder())){
                	attrs.put("order", operator.getOrder());
                }
                if(Util.isNotBlank(operator.getIsregister())){
                	attrs.put("isregister", operator.getIsregister());
                }
        ...
        try {
            ...
            for(int i=0;i<keys.length;i++){
            	...
            	if(values[i] instanceof List){
            		...
            		continue;
            	}else if("userpassword".equals(keys[i])){
            		if(values[i].toString().indexOf("{SH")!=-1){
                		continue;
            		}else{
            			attr = new BasicAttribute(keys[i], LDAPSecurityUtils.getOpenLDAPSHA(values[i].toString()));
            		}
            	}else{
			...
			for(SearchResult sr:list){
                ...
                while (ane.hasMore()) {
                    ...
                    while (values.hasMore()) {
                        ...
                        }else if(attrType.equals("orgid")){
                    	    operator.setOrgid(val);
                        ...
                        }else if(attrType.equals("numberplate")){
                    	    operator.setNumberplate(val);
                        }else if(attrType.equals("customerid")){
                    	    operator.setCustomerid(val);
                        }else if(attrType.equals("order")){
                    	    operator.setOrder(val);
                        }else if(attrType.equals("isregister")){
                    	    operator.setIsregister(val);
			...
			for(SearchResult sr:list){
                ...
                while (ane.hasMore()) {
                    ...
                    while (values.hasMore()) {
                        ...
                        }else if(attrType.equals("coordinates")){
                        	company.setCoordinates(val);
                        }else if(attrType.equals("phone")){
                        	company.setPhone(val);
                        }else if(attrType.equals("phone2")){
                        	company.setPhone2(val);
                        }else if(attrType.equals("phone3")){
                        	company.setPhone3(val);
                        }else if(attrType.equals("major")){
                        	company.setMajor(val);
                        }else if(attrType.equals("aptitude")){
                        	company.setAptitude(val);
                        }else if(attrType.equals("image")){
                        	company.setImage(val);
                        }else if(attrType.equals("image2")){
                        	company.setImage2(val);
                        }else if(attrType.equals("remark")){
                        	company.setRemark(val);
                        }
                    }
                }
     ...
     * @return
     */
    public CommonOperator getOperatorByopname(String opname) {
    	List<CommonOperator> list = searchOperator("", "(&(objectclass=commonOperator)(opname="+opname+"))");
    	if(list.size()>0){
    		return list.get(0);
    	}else{
    		return null;
     ...
     * @return
     */
    public CommonOperator getOperatorByMobile(String mobile) {
    	List<CommonOperator> list = searchOperator("", "(&(objectclass=commonOperator)(mobile="+mobile+"))");
    	if(list.size()>0){
    		return list.get(0);
    	}else{
    		return null;
     ...
     * @return
     */
    public CommonOperator getOperatorByMail(String mail) {
    	List<CommonOperator> list = searchOperator("", "(&(objectclass=commonOperator)(mail="+mail+"))");
    	if(list.size()>0){
    		return list.get(0);
    	}else{
    		return null;
     ...
     * @return
     */
    public CommonOperator getOperatorBycustId(String custId) {
    	List<CommonOperator> list = searchOperator("", "(&(objectclass=commonOperator)(customerid="+custId+"))");
    	if(list.size()>0){
    		return list.get(0);
    	}else{
    		return null;
    ...
    public List<CommonCompany> getOrgIdsByCompanyId(String companyId) {
    	...
    	for(CommonCompany company:list){
    		...
    		if("10".equals(company.getCompanytype())||"11".equals(company.getCompanytype())){
    			continue;
    		}
    		String dn = company.getDn();
     ...
     * @throws
     */
    public CommonOperator isExistSaleManagers(String companyno, String opname) {
    	List<CommonCompany> companynos = getOrgIdsByCompanyId(companyno);
    	String filter = "(&(objectclass=CommonOperator)(opname="+opname+")(roleid="+SystemConst.ROLEID_SALES_MANAGER+")(|(companynos="+companyno+")";
     ...
     * @return
     */
    public List<CommonOperator> getOperatorsByCompanyno(String companyno) {
    	String filter = "(&(objectclass=CommonOperator)(companynos="+companyno+"))";
    	List<CommonOperator> list = searchOperator("", filter);
    	return list;
    }
    ...
    public Page<CommonOperator> getPageSaleManager(PageSelect<HashMap<String, Object>> pageSelect, String companyno){
    	...
    	String filter = "";
    	if("2".equals(companyno)){
    		filter = "(&(objectclass=CommonOperator)(roleid="+SystemConst.ROLEID_SALES_MANAGER+"))";
    	}else{
	    	List<CommonCompany> companynos = getOrgIdsByCompanyId(companyno);
	    	filter = "(&(objectclass=CommonOperator)(roleid="+SystemConst.ROLEID_SALES_MANAGER+")(|(companynos="+companyno+")";
	    	for(CommonCompany company:companynos){
	    		filter += "(companynos="+company.getCompanyno()+")";
	    	}
	    	filter += "))";
     ...
     * @return
     */
    public CommonCompany getCompanyByPhone3(String phone3) {
    	List<CommonCompany> list = searchCompany("", "(&(objectclass=CommonCompany)(phone3="+phone3+"))");
    	if(list.size()>0){
    		return list.get(0);
    	}
    	return null;
    ...
    public CommonCompany getWarehouseByUserId(String userId) {
    	...
    	List<String> list = operator.getCompanynos();
     ...
     * @return
     */
    public CommonCompany getCompanyByname(String companyname) {
    	List<CommonCompany> list = searchCompany("", 
    			"(&(objectclass=CommonCompany)(companyname="+companyname+"))");
    	if(list!=null && list.size()>0){
    		return list.get(0);
    	}
    	return null;
    ...
    public static void main(String[] args) {
    	...
    	List<CommonCompany> list = ldap.searchCompany("", "(&(objectclass=CommonCompany)(phone3=*13911112222))");
