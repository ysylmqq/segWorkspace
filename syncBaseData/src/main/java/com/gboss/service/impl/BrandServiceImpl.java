...
public class BrandServiceImpl extends BaseServiceImpl implements BrandService {
	...
	public List<HashMap<String, Object>> brandTree(String keyword) {
				...
				for (Series series : seriesList) {
					...
					if (null != modelList && modelList.size() > 0) {
						...
						for (Model model : modelList) {
							...
							model_map.put("id", model.getModel_id());
					...
					series_map.put("id", series.getSeries_id());
	...
	public Brand getBrandName(String name) {
		return brandDao.getBrandName(name);
	}
