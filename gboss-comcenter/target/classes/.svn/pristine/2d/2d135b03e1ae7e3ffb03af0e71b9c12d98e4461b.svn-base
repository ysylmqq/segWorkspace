...
public class GetHistoryInfoACKHandler extends serverBaseHandler {

	public boolean lastpage = false;
	public String callLetter = "";
	public long historycount = 0;
	
	public GetHistoryInfoACKHandler(ComCenterBaseMessage basemsg, WebSocketClientHandler handler) {
	...
	public void run() {
		if (retcode == ResultCode.OK) {
			...
			StressTestDlg.historyoktimes.addAndGet(historycount);
			if (lastpage) {
				handler.historysn = "";
			}
		} else {
			StressTestDlg.historyerrortimes.incrementAndGet();
		}
	}
