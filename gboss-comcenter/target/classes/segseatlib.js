...
var segseatlib = {
	...
	message_type: {
        ...
        DeliverFaultLight: 2012,      //上传故障灯状态
        DeliverECUConfig: 2013,		  //上传ECU配置
	...
	*/
	...
	jsclient: function(_id, _hosts, _username, _password, _seatid, _usertype, _userversion) {
		 ...
		 */
		this.getHistoryInfo = function(callletter, infotype, starttime, endtime, pageNumber, totalNumber, autonextpage, sn, norepeat, reversed) {
			...
			var tmpobj = new tmpbuilder(callletter, infotype, startseconds, endseconds, pageNumber, totalNumber, autonextpage, sn, reversed, norepeat);
		...
		*************************************************************************************/
		var processMessage = function(basemsg) {
			...
			case segseatlib.message_type.DeliverUnitVersion:
				...
				break;
			//故障灯在GPS内，没有单独的结构
			//case segseatlib.message_type.DeliverFaultLight:
			//	processDeliverFaultLight(basemsg.content);
			//	break;
			case segseatlib.message_type.DeliverECUConfig:
				processDeliverECUConfig(basemsg.content);
				break;
		...
		var processGetLastInfoACK = function(content) {
			try {
				...
				if (lastack.gpses.length > 0 && typeof(ongetlastgps) != "undefined"){
					ongetlastgps(selfclient, lastack.retcode, lastack.retmsg, lastack.gpses, lastack.timestamps);
				}
				if (lastack.travels.length > 0 && typeof(ongetlasttravel) != "undefined"){
					ongetlasttravel(selfclient, lastack.retcode, lastack.retmsg, lastack.travels, lastack.timestamps);
				}
				if (lastack.faults.length > 0 && typeof(ongetlastfault) != "undefined"){
					ongetlastfault(selfclient, lastack.retcode, lastack.retmsg, lastack.faults, lastack.timestamps);
				}
				if (lastack.operates.length > 0 && typeof(ongetlastoperate) != "undefined"){
					ongetlastoperate(selfclient, lastack.retcode, lastack.retmsg, lastack.operates, lastack.timestamps);
				}
				if (lastack.sms.length > 0 && typeof(ongetlastsm) != "undefined"){
					ongetlastsm(selfclient, lastack.retcode, lastack.retmsg, lastack.sms, lastack.timestamps);
				}
				if (lastack.alarms.length > 0 && typeof(ongetlastalarm) != "undefined"){
					ongetlastalarm(selfclient, lastack.retcode, lastack.retmsg, lastack.alarms, lastack.timestamps);
				}
				if (lastack.obds.length > 0 && typeof(ongetlastobd) != "undefined"){
					ongetlastobd(selfclient, lastack.retcode, lastack.retmsg, lastack.obds, lastack.timestamps);
				}
				if (lastack.unitloginout.length > 0 && typeof(ongetlastunitloginout) != "undefined"){
					ongetlastunitloginout(selfclient, lastack.retcode, lastack.retmsg, lastack.unitloginout, lastack.timestamps);
				}
		...
		var processGetHistoryInfoACK = function(content) {
			try {
				...
				if (historyack.gpses.length > 0 && typeof(ongethistorygps) != "undefined"){
					ongethistorygps(selfclient, historyack.retcode, historyack.retmsg, historyack.lastPage, historyack.gpses, historyack.timestamps);
				}
				if (historyack.travels.length > 0 && typeof(ongethistorytravel) != "undefined"){
					ongethistorytravel(selfclient, historyack.retcode, historyack.retmsg, historyack.lastPage, historyack.travels, historyack.timestamps);
				}
				if (historyack.faults.length > 0 && typeof(ongethistoryfault) != "undefined"){
					ongethistoryfault(selfclient, historyack.retcode, historyack.retmsg, historyack.lastPage, historyack.faults, historyack.timestamps);
				}
				if (historyack.operates.length > 0 && typeof(ongethistoryoperate) != "undefined"){
					ongethistoryoperate(selfclient, historyack.retcode, historyack.retmsg, historyack.lastPage, historyack.operates, historyack.timestamps);
				}
				if (historyack.sms.length > 0 && typeof(ongethistorysm) != "undefined"){
					ongethistorysm(selfclient, historyack.retcode, historyack.retmsg, historyack.lastPage, historyack.sms, historyack.timestamps);
				}
				if (historyack.alarms.length > 0 && typeof(ongethistoryalarm) != "undefined"){
					ongethistoryalarm(selfclient, historyack.retcode, historyack.retmsg, historyack.lastPage, historyack.alarms, historyack.timestamps);
				}
				if (historyack.obds.length > 0 && typeof(ongethistoryobd) != "undefined"){
					ongethistoryobd(selfclient, historyack.retcode, historyack.retmsg, historyack.lastPage, historyack.obds, historyack.timestamps);
				}
				if (historyack.unitloginout.length > 0 && typeof(ongethistoryunitloginout) != "undefined"){
					ongethistoryunitloginout(selfclient, historyack.retcode, historyack.retmsg, historyack.lastPage, historyack.unitloginout, historyack.timestamps);
				}
		...
		var processGetHistorySimpleGpsInfoACK = function(content) {
			try {
				...
				if (historyack.gpses.length > 0 && typeof(ongethistorysimplegps) != "undefined"){
					ongethistorysimplegps(selfclient, historyack.callLetter, historyack.retcode, historyack.retmsg, historyack.lastPage, historyack.gpses, historyack.timestamps);
				}
				if (historyack.retcode != 0 && typeof(ongethistoryerror) != "undefined") {
					ongethistoryerror(selfclient, historyack.retcode, historyack.retmsg);
				}
		...
		var processDeliverECUConfig = function(content) {
			try {
				var deliver = segseatlib.builder.build("DeliverECUConfig");
				deliver = deliver.decode(content);
				//调用外部函数
				if (typeof(ondeliverecuconfig) != "undefined") {
					ondeliverecuconfig(selfclient, deliver.ecuConfig, deliver.gatewayid, deliver.gatewaytype);
				}
		    } catch (err) {
		    	SEGUtil_WS.showError("DeliverFault error:" +err);
		    }
