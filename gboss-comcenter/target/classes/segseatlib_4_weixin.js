...
var segseatlib = {
	...
	decode: function(recv) {
		try {
			...
			//判断长度
			var tmplen = SEGUtil_WS.getShortFromByteArray(recv, 4);
			if (tmplen != recv.length) {
				return null;
			}
			//判断crc
	...
	*/
	...
	jsclient: function(_id, _hosts, _username, _password, _seatid, _usertype, _userversion) {
		...
		this.send = function(buff) {
			...
			//如果不是字节串, 直接发送
			if (typeof(buff.length) != "undefined"){
		...
		var processGetLastInfoACK = function(content) {
			try {
				...
				if (lastack.unitloginout.length > 0 && typeof(ongetlastunitloginout) != "undefined"){
					ongetlastunitloginout(selfclient, lastack.retcode, lastack.retmsg, lastack.unitloginout);
				}
		...
		var processGetHistoryInfoACK = function(content) {
			try {
				...
				if (historyack.unitloginout.length > 0 && typeof(ongethistoryunitloginout) != "undefined"){
					ongethistoryunitloginout(selfclient, historyack.retcode, historyack.retmsg, historyack.lastPage, historyack.unitloginout);
				}
