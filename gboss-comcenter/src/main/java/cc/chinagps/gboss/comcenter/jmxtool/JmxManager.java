...
public class JmxManager {
	...
	public void start() {
		try {
	        ...
	        //创建MBean
	        GbossControl control = new GbossControl("JMX Thread");
	        //将MBean注册到MBeanServer中
	        mbs.registerMBean(control, new ObjectName("GBossComcenterMBean:name=Controller"));
	        control.start();
	        
	        //创建适配器，用于能够通过浏览器访问MBean
	        HtmlAdaptorServer adapter = new HtmlAdaptorServer();
        	adapter.setPort(jmxport);
	        mbs.registerMBean(adapter, new ObjectName("GBossComcenterMBean:name=htmlController(" + jmxport + ")"));
	        adapter.start();
