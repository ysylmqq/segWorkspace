...
function askSend(){
	...
	var contentText = editor.document.getBody().getText(); //取得纯文本
	//alert(content);
	//alert(contentText);
	//return;
	if(title==null || title==""){
	...
	//alert(content);
  		...
  		$.ajax({
			   type : "POST",
			   data: {title:title,content:content,contentText:contentText}, 
			   url : "userInfo/message_messageSaveOrUpdate.action",
...
function showDetail(obj){
	$('#hid').val(obj);
	$.post("userInfo/message_getById.action", {
  	  id:obj
				}, function(data) {
...
function formatterText(val,row,index){
	var id = row.id;
	var str= val+'&nbsp;'+'<a  style="color: blue;text-decoration: underline;cursor: pointer;" &nbsp;'+'<a  style="color: blue;text-decoration: underline;cursor: pointer;" onclick=del('+id+')>删除</a>';;
	return str;
}
...
function del(obj){
	 $.messager.confirm(tipMsgDilag,"确认删除?",
  		...
  		$.ajax({
			   type : "POST",
			   data: {id:obj}, 
			   url : "userInfo/message_del.action",
			   error : function(jqXMLRequest, textStatus, errorThrown){
				   $.messager.alert(tipMsgDilag, "错误类型：" + textStatus + "<br />错误信息：" + errorThrown, 'error');
			   },
			   success : function(result){
...
function askEditSend(){
	...
	//alert(content);
  		...
  		$.ajax({
			   type : "POST",
			   data: {title:title,content:content,id:$('#hid').val()}, 
			   url : "userInfo/message_projectInfoEditProjectInfo.action",
			   error : function(jqXMLRequest, textStatus, errorThrown){
				   $.messager.alert(tipMsgDilag, "错误类型：" + textStatus + "<br />错误信息：" + errorThrown, 'error');
			   },
			   success : function(result){
