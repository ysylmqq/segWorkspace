...
var drawRectCallBack = function(val, rect, top, xy){
   ...
   //alert(lon+","+lat+","+lon2+","+lat2);
	...
	if(typeof parent.window.openRectWin=='function'){
		parent.window.openRectWin(params,rect);
	}
}
//删除矩形
function removeRecMarker(rect){
	maplet.removeMarker(rect);
}
//获得地图级别
...
//显示区域及机械
function showAreaAndVehicles(lon,lat,lon2,lat2,areapointId,areapointName,mlevel){
	maplet.clearMarkers();
	var p1 = new LatLng(lat,lon);
	var p2 = new LatLng(lat2,lon2);
	var rect = new Rect(p1,p2,2, "red",null,areapointId);
	maplet.addMarker(rect);
	maplet.setCenter((lat + lat2)/2, (lon + lon2)/2);
	maplet.setZoomLevel(mlevel);
	
	existRects.push(rect);
	//查询区域
	var params={
			'condition.lon':lon,
			'condition.lat':lat,
	...
	$.post("${basePath}run/run_getVechilesInArea.action", params, function(data){
        if(data){
       		if (trace) {
				trace.clear();
				trace = null;
			...
			//添加到表格中
			if(typeof parent.window.addVehiclesToGrid=='function'){
				parent.window.addVehiclesToGrid(data,areapointName);
			}
        }
        }, 'json');
}
...
//显示区域及机械
function showAreaAndVehicles1(lon,lat,lon2,lat2,areapointId,areapointName,mlevel){
	maplet.clearMarkers();
	var p1 = new LatLng(lat,lon);
	var p2 = new LatLng(lat2,lon2);
	var rect = new Rect(p1,p2,2, "red",null,areapointId);
...
function clearRects(){
	maplet.clearMarkers();
	while(existRects.length > 0){
		maplet.removeMarker(existRects.pop());
	}
}
	</script>
