...
<div class="easyui-layout" data-options="fit:true,border:false" style="width:100%;height:100%;">
   <div data-options="region:'center',border:false,onResize:vehicleResize"  style="overflow:hidden;">
    ...
    <table id="customer_pay_datagrid"  toolbar="#toolbar" class="easyui-datagrid"
			...
			data-options="url:'${basePath}sim/companyPay_search.action',
            fitColumns:true,singleSelect:true,rownumbers:true,pagination:true,title:'公司缴费'">
             <thead>  
            <tr>  
		        ...
		        <th data-options="field:'simPayId',width:80,align:'center',formatter:operateTool">操作</th>
	            </tr>  
        </thead>
	 </table>
		<!-- 表格 end -->
      ...
      <td  align="right">
        ...
        <td>
        	<input id="sim_no" name="simNo" class="easyui-combobox"
							style="width: 200px;"
							data-options="
							url:'${basePath}sim/companyPay_simList.action',
								valueField:'simNo',  
       							textField:'simNo',
 ...
 <!-- 批量导入弹出框 begin-->
  <div id="customerPayImpport" class="easyui-dialog" title="批量导入" data-options="iconCls:'icon-save',modal:true"
        ...
        <form id="vehicle_impport_Form" method="post" theme="simple"
			enctype="multipart/form-data" action="${basePath}sim/companyPay_impFromExcel.action" >
  ...
  <!-- 新增弹出框 begin-->
  <div id="customerPayWindow" class="easyui-dialog" title="公司缴费" data-options="iconCls:'icon-save',modal: true" style="padding: 5px; width: 600px; height: 360px;" closed="true" buttons="#operBtns">
	...
	if(objtype){
       ...
       if(flag){
           $('#vehicle_impport_Form').form('submit', { 
				url: '${basePath}sim/companyPay_impFromExcel.action', 
				onSubmit: function(){ 
...
function customerPayDel(val){
	 ...
	 $.messager.confirm(tipMsgDilag, "确定删除?",function(r) {
	    if (r) {
	        $.post("${basePath}sim/companyPay_delCompanyPayPOJOById.action", {simPayId:val},function(data) {
 ...
 function customerPayEdit(val){
	 ...
	 $("#custPayId").val(val);
	 $.post('${basePath}sim/companyPay_getCustomerPayPOJOById.action',{custPayId:val},function(result){
      ...
      var customerPay = {
        init: function() {
            ...
            $('#simNo').combobox({
				    ...
				    valueField:'simNo',  
       				...
       				onChange:function(data){
       					$.post("${basePath}sim/companyPay_companySimById.action",{simNo:data},function(result){
       						if(result != null  && result != ""){
       							var obj = JSON.parse(result); 
		  ...
		  $("#saveCustomerPayBtn").click(function(){
		  		var simNo = $("#simNo").closest("td").find("input[type='hidden']").val();
		  		var payAmount = $("#payAmount").val();
		  		if($("#endTime").val() == "" || $("#endTime").val() == null ){
		  		    $.messager.alert(tipMsgDilag, "服务结束日期必填!");
		  			return; 
		  		}
		  		var flag1 = simNoValidate(simNo);
		  		if(flag1){
			  		...
			  		if(flag1 && flag1){
			  			...
			  			if(simNo !== null && simNo != "" && payAmount != null && payAmount != ""){
				  			$.post('${basePath}sim/companyPay_companyPay.action',data,function(result){
				  				if( result == "1"){
				  					$("#queryBtn").click();
				  					...
				  					$.messager.alert(tipMsgDilag, "程序内部出现错误");
				  				}
				  			});
			  			}
			  		}
...
function downExcelVehicle(){
	...
	window.location.href=encodeURI(encodeURI("${basePath}sim/companyPay_exportToExcelVehicle.action?simNo="+simNo+
							"&startTime="+startTime+
							"&endTime="+endTime+
						    "&payAmount="+payAmount+
						    "&dealerId="+$('#dealerId_search').combobox('getValues').join(',')+
