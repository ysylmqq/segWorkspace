...
function operateTool(val,row,index){
	...
	if(row.isServer == "" ||row.isServer == null){
		...
		if(status == "1"){
			operStr = '<a style="color:blue;" class="customerPayEditBtn" href="javascript:void(0)" onclick="cancelStopServer(\''+simNo+'\')" >解除注销</a>';
		}else{
			operStr = '<a style="color:blue;" class="customerPayEditBtn" href="javascript:void(0)" onclick="stopServer(\''+simNo+'\')" >注销</a><a style="color:blue;margin-left:5px" class="customerPayEditBtn" href="javascript:void(0)" onclick="customerPayEdit(\''+simNo+'\')" >编辑</a>';
		}
	}
	
	if( row.vehicleDef == "" || row.vehicleDef  == null){
		operStr += '<a style="color:blue;margin-left:5px" class="customerPayEditBtn" href="javascript:void(0)" onclick="companySimDelBtn(\''+simNo+'\')" >删除</a> ';
	}
	
	return operStr;
}

function companySimDelBtn( simNo ){
$.messager.confirm(tipMsgDilag, "确定删除?",function(r) {
	    if (r) {
	        $.post("${basePath}sim/companySim_delBySImNo.action", {simNo:simNo},function(data) {
	            var obj = $.parseJSON(data);
	            if(obj.success){
	             	$("#queryBtn").click();
