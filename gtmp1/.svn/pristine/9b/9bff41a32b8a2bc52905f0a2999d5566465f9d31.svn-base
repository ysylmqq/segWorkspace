//窗口大小变化
function upUploadResize (w,h){
	if($('#upUpload_query_datagrid')){
		try{
		  $('#upUpload_query_datagrid').datagrid('options');
...
//查询
function queryUpUpload(){
	if(!$('#days_search').combobox('getValue')){
	      $.messager.alert('提示','请选择未上报的天数!!');
		   return;
	}
	var param={
	   ...
	   vehicleArg: $('#vehicleArg_search').combobox('getValue') == '全部' ? '' : $('#vehicleArg_search').combobox('getValue')
		...
		try{
        			if($('#upUpload_query_datagrid').datagrid('options').url){
        			  $('#upUpload_query_datagrid').datagrid('load', param);
        			}else{
        			  $('#upUpload_query_datagrid').datagrid({
		        		  url:'report/statistic_getUnUploadByPage.action',
		        		  queryParams:param
		        		});
        			}
        		}catch(e){
...
function downExcel(){
	...
	//alert("report/statistic_exportToExcelUnUpload.action?modelId="+vehicleModel+"&vehicleCode="+vehicleCode+"&vehicleArg="+vehicleArg+"&direction="+$('#days_search').combobox('getValue')+"&vehicleDef="+$('#vehicleDef_search').val()+"&dealerId="+$('#dealerId_search').combobox('getValues').join(','));
	window.location.href = encodeURI(encodeURI("report/statistic_exportToExcelUnUpload.action?modelId="
			+ vehicleModel
			+ "&vehicleStatus="
			+ vehicleStatus
			+ "&vehicleCode="
			...
			+ vehicleArg
			+ "&direction="
			+ $('#days_search').combobox('getValue')
			+ "&vehicleDef="
			+ $('#vehicleDef_search').val()
			+ "&dealerId="
			+ $('#dealerId_search').combobox('getValues').join(',')));
	return;
}
