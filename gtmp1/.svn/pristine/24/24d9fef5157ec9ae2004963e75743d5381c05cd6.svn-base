                 ...
                 onBeforeExpand:function(node,param){  
                 	if(node.attributes){//异步加载
                 	   if(node.attributes.dtype){
                 	     	$('#run_tree').tree('options').url = "run/owner_getDealerAreasList.action?dealerAreaPOJO.dtype="+node.attributes.dtype;
                 	   }
                 	}
                 	// 
                 },               
	...
	var text=$('#run_tree_text').val();
	 if(text==run_search_text){//如果是没有任何条件的查询
		 ...
		 if((params.dealerIds&&params.dealerIds.length>0)||params.typeId||params.modelId||params.unitSn||params.steelNo||params.materialNo||params.isValid||params.testFlag){
		 	 ...
		 	 $('#run_tree').tree('options').url = "run/owner_getDealerAreasList.action";
	 ...
	 params.vehicleDef=text;
       ...
       { 
        type: "POST", 
        url: "run/owner_getDealerAreasListXx.action", 
...
function openQueryDialog(){
	//经销商列表
	 $('#dealerId').combotree({
	 url:'run/owner_getDealerAreas4Tree.action'
     });
     //机械类型
      $('#typeId').combobox({
...
function addMonitor(){
   ...
   data= $('#grid_run_monitor').datagrid('getData');
	for (var index = 0; index < data.total; index++) {
	  ...
	  $.post('run/owner_addMonitorList.action',
...
function clearMonitorData(handler){
	...
	if(handler==1){
       ...
       //从session中移除
	   $.post('run/owner_delWatchUnitIds.action',
      			$.param({vehicleUnitPOJOs:monitorList},true),
    			function(result){
		...
		for (var index = 0; index < data.total; ) {
    	 ...
    	 //从session中清除
	   $.post('run/owner_cleanWatchVehicle.action',
    			function(result){
...
function openParamsSetWin(){
	...
	defaultVals['pCanCommand']=["0x00AC","CAN ID号:","0","数据:"];//CAN数据透传指令
...
function refreshResponsInfo(){
	//从memcache中取数据
	 $.post('run/owner_getMemcacheData.action',
    			function(result){
...
function distributeInDealer(){
	if(selectedDealerId!=null){
		 $.ajax( 
			       { 
			        type: "POST", 
			        url: "run/owner_getVehicleDestribute.action", 
...
function refreshGpsWorkOnLine(){
	//从memcache中取数据
	 $.post('run/owner_getGpsWorkOnLineData.action',
    			function(result){
      	  	     try {
						var r = $.parseJSON(result);
						var gpsInfo=r.gpsInfo;//gps信息
...
//打开在线、离线窗口
function openOnlineWin(type){
	var queryParams={"isValid":type};
	try{
		...
		var title = "在线";
		var iconCls = "icon-car";
		if(type == 1){
			title = "离线";
			iconCls = "icon-black-car";
		}
		$('#dlg_online_run').dialog({'title':title,'iconCls':iconCls});
...
function queryOnline(){
	var title=$('#dlg_online_run').attr('title');
	var isLogin=0;
	if(title=="离线"){
		isLogin=1;
	}
...
function openWorkHoursWin(){
	...
	$('#dlg_workhours').dialog('open');
 	$.ajax( 
		       { 
		        type: "POST", 
		        url: "run/owner_getLastWorkInfo.action", 
...
function refreshRunTrack(){
	...
	//从memcache中取数据
	 $.post('run/owner_getTrackData.action',
