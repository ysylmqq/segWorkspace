...
# sql语句
sql1 = "SELECT * FROM t_hm_faultinfo WHERE starttime >= '2016-10-01 00:00:00' and starttime < '2016-11-16 00:00:00' ORDER BY CALL_LETTER"
...
# 打开一个workbook
workbook = xlrd.open_workbook('D:\\files\\海马体检统计信息输入.xlsx')
worksheets = workbook.sheet_names()
print('worksheets is %s' % worksheets)
# 定位到sheet1
...
for curr_row in range(num_rows):
    rowxls = worksheet1.row_values(curr_row)
    # TBOX故障信息数组赋值为空
    tbox_time = []
    tbox_faultinfo = []
    tbox_faulname = []
    ...
    for row4 in rows4:  # 在t_hm_sim
        if rowxls[1] == row4[1]:#利用vin码进行遍历
            sheet.write(curr_row + 2, 0, curr_row)
            sheet.write(curr_row+2, 5, row4[1])
            sheet.write(curr_row+2, 1, row4[2])
            sheet.write(curr_row+2, 2, row4[3])
            ...
            for row1 in rows1:
                if row1[3]==row4[0]:
                    if flag !=row1[3]:
                        flag = row1[3]
                        flag2 = 0
                        sheet.write(curr_row+2, 6, row1[3])
                        if curr_row>1:#每一个字段换行输出
                                sheet.write(curr_row+1, 11, '\n'.join(tbox_time))  #TBOX
                                sheet.write(curr_row+1, 12, '\n'.join(tbox_faultinfo))
                                sheet.write(curr_row+1, 13, '\n'.join(tbox_faulname))

                                sheet.write(curr_row+1, 14, '\n'.join(ems_time))  # EMS
                                sheet.write(curr_row+1, 15, '\n'.join(ems_faultinfo))
                                sheet.write(curr_row+1, 16, '\n'.join(ems_faulname))

                                sheet.write(curr_row+1, 17, '\n'.join(tcu_time))  # TCU
                                sheet.write(curr_row+1, 18, '\n'.join(tcu_faultinfo))
                                sheet.write(curr_row+1, 19, '\n'.join(tcu_faulname))

                                sheet.write(curr_row+1, 20, '\n'.join(abs_esp_time))  # ABS_ESP
                                sheet.write(curr_row+1, 21, '\n'.join(abs_esp_faultinfo))
                                sheet.write(curr_row+1, 22, '\n'.join(abs_esp_faulname))

                                sheet.write(curr_row+1, 23, '\n'.join(tpms_time))  # TPMS
                                sheet.write(curr_row+1, 24, '\n'.join(tpms_faultinfo))
                                sheet.write(curr_row+1, 25, '\n'.join(tpms_faulname))

                                sheet.write(curr_row+1, 26, '\n'.join(bcm_time))  # BCM
                                sheet.write(curr_row+1, 27, '\n'.join(bcm_faultinfo))
                                sheet.write(curr_row+1, 28, '\n'.join(bcm_faulname))

                                sheet.write(curr_row+1, 29, '\n'.join(immo_time))  # IMMO
                                sheet.write(curr_row+1, 30, '\n'.join(immo_faultinfo))
                                sheet.write(curr_row+1, 31, '\n'.join(immo_faulname))

                                sheet.write(curr_row+1, 32, '\n'.join(apm_time))  # APM
                                sheet.write(curr_row+1, 33, '\n'.join(apm_faultinfo))
                                sheet.write(curr_row+1, 34, '\n'.join(apm_faulname))

                                sheet.write(curr_row+1, 35, '\n'.join(icm_time))  # ICM
                                sheet.write(curr_row+1, 36, '\n'.join(icm_faultinfo))
                                sheet.write(curr_row+1, 37, '\n'.join(icm_faulname))

                                sheet.write(curr_row+1, 38, '\n'.join(eps_time))  # EPS
                                sheet.write(curr_row+1, 39, '\n'.join(eps_faultinfo))
                                sheet.write(curr_row+1, 40, '\n'.join(eps_faulname))

                                sheet.write(curr_row+1, 41, '\n'.join(peps_time))  # PEPS
                                sheet.write(curr_row+1, 42, '\n'.join(peps_faultinfo))
                                sheet.write(curr_row+1, 43, '\n'.join(peps_faulname))

                                sheet.write(curr_row+1, 44, '\n'.join(srs_time))  # SRS
                                sheet.write(curr_row+1, 45, '\n'.join(srs_faultinfo))
                                sheet.write(curr_row+1, 46, '\n'.join(srs_faulname))
                    ...
                    if flag2==0:#同一个callletter
                            ...
                            if len(nodeLostInfo)==0:
                                sheet.write(curr_row+2, 7, 0)
                            else:
                                sheet.write(curr_row+2, 7, ','.join(nodeLostInfo))

                            if len(nodeFaultInfo)==0:
                                sheet.write(curr_row+2, 8, 0)
                            else:
                                sheet.write(curr_row+2, 8, ','.join(nodeFaultInfo))

                            sheet.write(curr_row+2, 9, start_gpstime)
                            sheet.write(curr_row+2, 10, end_gpstime)
                            # print(count-2,row1[3], start_gpstime,nodeLostInfo, nodeFaultInfo,end_gpstime)

            #输出最后一行的故障信息，每一个字段换行输出
            sheet.write(curr_row+2, 11, '\n'.join(tbox_time))#TBOX
            sheet.write(curr_row+2, 12, '\n'.join(tbox_faultinfo))
            sheet.write(curr_row+2, 13, '\n'.join(tbox_faulname))

            sheet.write(curr_row+2, 14, '\n'.join(ems_time))  # EMS
            sheet.write(curr_row+2, 15, '\n'.join(ems_faultinfo))
            sheet.write(curr_row+2, 16, '\n'.join(ems_faulname))

            sheet.write(curr_row+2, 17, '\n'.join(tcu_time))  # TCU
            sheet.write(curr_row+2, 18, '\n'.join(tcu_faultinfo))
            sheet.write(curr_row+2, 19, '\n'.join(tcu_faulname))

            sheet.write(curr_row+2, 20, '\n'.join(abs_esp_time))  # ABS_ESP
            sheet.write(curr_row+2, 21, '\n'.join(abs_esp_faultinfo))
            sheet.write(curr_row+2, 22, '\n'.join(abs_esp_faulname))

            sheet.write(curr_row+2, 23, '\n'.join(tpms_time))  # TPMS
            sheet.write(curr_row+2, 24, '\n'.join(tpms_faultinfo))
            sheet.write(curr_row+2, 25, '\n'.join(tpms_faulname))

            sheet.write(curr_row+2, 26, '\n'.join(bcm_time))  # BCM
            sheet.write(curr_row+2, 27, '\n'.join(bcm_faultinfo))
            sheet.write(curr_row+2, 28, '\n'.join(bcm_faulname))

            sheet.write(curr_row+2, 29, '\n'.join(immo_time))  # IMMO
            sheet.write(curr_row+2, 30, '\n'.join(immo_faultinfo))
            sheet.write(curr_row+2, 31, '\n'.join(immo_faulname))

            sheet.write(curr_row+2, 32, '\n'.join(apm_time))  # APM
            sheet.write(curr_row+2, 33, '\n'.join(apm_faultinfo))
            sheet.write(curr_row+2, 34, '\n'.join(apm_faulname))

            sheet.write(curr_row+2, 35, '\n'.join(icm_time))  # ICM
            sheet.write(curr_row+2, 36, '\n'.join(icm_faultinfo))
            sheet.write(curr_row+2, 37, '\n'.join(icm_faulname))

            sheet.write(curr_row+2, 38, '\n'.join(eps_time))  # EPS
            sheet.write(curr_row+2, 39, '\n'.join(eps_faultinfo))
            sheet.write(curr_row+2, 40, '\n'.join(eps_faulname))

            sheet.write(curr_row+2, 41, '\n'.join(peps_time))  # PEPS
            sheet.write(curr_row+2, 42, '\n'.join(peps_faultinfo))
            sheet.write(curr_row+2, 43, '\n'.join(peps_faulname))

            sheet.write(curr_row+2, 44, '\n'.join(srs_time))  # SRS
            sheet.write(curr_row+2, 45, '\n'.join(srs_faultinfo))
            sheet.write(curr_row+2, 46, '\n'.join(srs_faulname))


str_total='车辆总数：'+str(num_rows-1)
sheet1.write(2,4,str_total)
#ICM故障
sheet1.write(4,2,len(arr_icm))
sheet1.write(4,3,len(array_icm))
sheet1.write(4,4,len(callletter_icm))
sheet1.write(4,5,round(len(callletter_icm)/(num_rows)*100,2))
#APM故障
sheet1.write(5,2,len(arr_apm))
sheet1.write(5,3,len(array_apm))
sheet1.write(5,4,len(callletter_apm))
sheet1.write(5,5,round(len(callletter_apm)/(num_rows)*100,2))
#EMS故障
sheet1.write(6,2,len(arr_ems))
sheet1.write(6,3,len(array_ems))
sheet1.write(6,4,len(callletter_ems))
sheet1.write(6,5,round(len(callletter_ems)/(num_rows)*100,2))
#IMMO故障
sheet1.write(7,2,len(arr_immo))
sheet1.write(7,3,len(array_immo))
sheet1.write(7,4,len(callletter_immo))
sheet1.write(7,5,round(len(callletter_immo)/(num_rows)*100,2))
#TBOX故障
sheet1.write(8,2,len(arr_immo))
sheet1.write(8,3,len(array_immo))
sheet1.write(8,4,len(callletter_immo))
sheet1.write(8,5,round(len(callletter_immo)/(num_rows)*100,2))
#ABS_ESP故障
sheet1.write(9,2,len(arr_abs_esp))
sheet1.write(9,3,len(array_abs_esp))
sheet1.write(9,4,len(callletter_abs_esp))
sheet1.write(9,5,round(len(callletter_abs_esp)/(num_rows)*100,2))
#TPMS故障
sheet1.write(10,2,len(arr_tpms))
sheet1.write(10,3,len(array_tpms))
sheet1.write(10,4,len(callletter_tpms))
sheet1.write(10,5,round(len(callletter_tpms)/(num_rows)*100,2))
#BCM故障
sheet1.write(11,2,len(arr_bcm))
sheet1.write(11,3,len(array_bcm))
sheet1.write(11,4,len(callletter_bcm))
sheet1.write(11,5,round(len(callletter_bcm)/(num_rows)*100,2))
#TCU故障
sheet1.write(12,2,len(arr_tcu))
sheet1.write(12,3,len(array_tcu))
sheet1.write(12,4,len(callletter_tcu))
sheet1.write(12,5,round(len(callletter_tcu)/(num_rows)*100,))
#PEPS故障
sheet1.write(13,2,len(arr_peps))
sheet1.write(13,3,len(array_peps))
sheet1.write(13,4,len(callletter_peps))
sheet1.write(13,5,round(len(callletter_peps)/(num_rows)*100,2))
#EPS故障
sheet1.write(14,2,len(arr_eps))
sheet1.write(14,3,len(array_eps))
sheet1.write(14,4,len(callletter_eps))
sheet1.write(14,5,round(len(callletter_eps)/(num_rows)*100,2))
#SRS故障
sheet1.write(15,2,len(arr_srs))
sheet1.write(15,3,len(array_srs))
sheet1.write(15,4,len(callletter_srs))
sheet1.write(15,5,round(len(callletter_srs)/(num_rows)*100,2))
#计算故障总和
